...
class NotificacaoSchema(Schema):
    ...
    conteudoMensagem = fields.String(
        ...
        validate=validate.Length(
            min=1, error='conteudoMensagem não pode ser vazio'
        ),
    )
    tipoNotificacao = fields.String(
        ...
        validate=validate.OneOf(
            TIPOS_VALIDOS,
            error=f'tipoNotificacao deve ser um de {TIPOS_VALIDOS}',
        ),
    )
...
def _open_channel():
    ...
    channel.exchange_declare(
        exchange=EXCHANGE_NAME, exchange_type='direct', durable=False
    )
...
def send_message():
    ...
    try:
        ...
        return jsonify({
            'status': 'error',
            'message': f'Falha ao publicar no RabbitMQ: {e.__class__.__name__}',
        }), 502
...
def consultar_status(traceId: str):
    ...
    if not notificacao:
        return jsonify({
            'status': 'error',
            'message': f'traceId {traceId} não encontrado',
        }), 404
